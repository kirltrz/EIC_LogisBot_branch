...
typedef enum {
    /**
     * 使用缓冲区 (s) 来渲染屏幕较小的部分。
     * 这种方式的缓冲区可以更小，然后显示保存RAM。建议至少使用1/10屏幕大小缓冲区。
     */
    LV_DISPLAY_RENDER_MODE_PARTIAL,

    /**
     * 缓冲区必须是屏幕大小，LVGL将呈现到缓冲区的正确位置。
     * 这样的缓冲区总是包含整个图像。只有改变的战神将被更新。
     * 使用2个缓冲区，缓冲区的内容会自动保持同步，并且在flush_cb中只需要更改地址。
     */
    LV_DISPLAY_RENDER_MODE_DIRECT,

    /**
     * 总是重绘整个屏幕，即使只有一个像素已被改变。
     * 仅在flush_cb中有2个缓冲区，并且需要更改地址。
     */
